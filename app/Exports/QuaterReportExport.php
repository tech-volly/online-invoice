...
class QuaterReportExport implements FromCollection, WithHeadings, ShouldAutoSize, WithStyles, WithEvents, WithColumnFormatting
{
    /**
    ...
    */
    ...
    public function collection() {
        ...
        if($quarter == 'quarter4') {
            $start_date = $next_year.'-04-01';
            $end_date = $next_year.'-06-30';
        }
        $income_expense_arr = [];
        //$incomes = Invoice::with(['client', 'payment_status'])->whereBetween('invoice_payment_date', [$start_date, $end_date])->get();
        $incomes = Invoice::with(['client', 'payment_status'])
                        ->leftJoin('projects', 'invoices.project_id', '=', 'projects.id')
                        ->whereBetween('invoice_payment_date', [$start_date, $end_date])
                        ->select('invoices.*', 'projects.name as inc_project_name')
                        ->get();
        //$expenses = Expense::with(['supplier', 'payment_method'])->whereBetween('expense_date', [$start_date, $end_date])->get();
