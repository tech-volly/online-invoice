$(document).ready(function () {
    if($('#categoryDataTable').length > 0) {
        var categories = $('#categoryDataTable').DataTable({
            ...
            buttons: [
                {
                    extend: 'excel',
                    text: '<i class="las la-file-export"></i> Export Expense Categories',
    ...
    $('#categoryDataTable').on('click', '.editCategory', function () { 
        ...
        var url = base_url + 'master/expense/categories/edit/'+id;
        $.ajax({
            headers: {
    ...
    $('#importCategoryForm').validate({
        ...
        messages: {
            import_categories_file: {
                required: 'Please upload file to import expense categories',
    ...
    $('#categoryDataTable').on('click','.deleteCategoryBtn', function(e) {
        e.preventDefault();
        var id = $(this).attr('data-id');
        var url = base_url + 'master/expense/categories/delete/'+id;
        swal({
            title: "Are you sure?",
            ...
            if (willDelete) {
                $.ajax({
                    ...
                    if (data.success == 1) {
                        ...
                        swal("Oops! You can not delete this expense category as it is used in Supplier.", {
                            icon: "error",
                        }); 
                    } else {
    ...
    $('#btnAllDelete').on('click', function () {
        ...
        var url = base_url + 'master/expense/categories/delete-selected-records';
        swal({
            title: "Are you sure?",
