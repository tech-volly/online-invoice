$(document).ready(function () {
    ...
    $('.select_year_value').select2({
        dropdownParent: $('body') // keeps it stable
    });
    ...
    $('.clientDateTimePicker').datetimepicker({
        format: 'MM-YYYY',
        icons: {
            up: "fa fa-angle-up",
            down: "fa fa-angle-down",
    ...
    $(document).on('click', '#download_client_statement', function (e) {
        e.preventDefault();
        var from_date = $('#client_from_year_month').val();
        var to_date = $('#client_to_year_month').val();
        var client_id = $('#client_id').val();
        if (from_date == '' || to_date == '' || !client_id) {
            swal("Oops! Please select from date, to date and client to download report.", {
                icon: "error",
            });
            return false;
        }
        var url = $(this).attr('href') + '?from_date=' + from_date + '&to_date=' + to_date + '&client_id=' + client_id;
        window.location.href = url;
    });
});